...
class UNetChannelHead(nn.Module):
    ...
    def __init__(self, latent_dim: int, n_ant: int, n_sub: int, base: int = 48) -> None:
        super().__init__()
        ...
        # input: 2 (real/imag) + 1 (noise-var map) = 3 channels
        self.inc = _ConvBlock(3, base)
    ...
    def forward(self, obs: torch.Tensor, z: torch.Tensor, noise_var: torch.Tensor) -> torch.Tensor:
        b = obs.shape[0]
        ...
        x0 = self.inc(x)                                         # (B, base, A, S)
        x1 = self.down1(self.pool(x0))                          # (B, 2base, A/2, S/2)
        x2 = self.down2(self.pool(x1))                          # (B, 4base, A/4, S/4)
        x2 = self.film(x2, ctx)                                 # condition on latent + noise
