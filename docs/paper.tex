...
enabler} --- removing the 2-D DFT worsens estimation by $+248$--$298\%$ at every SNR, an order of
magnitude larger than any other effect; (ii) the \textbf{world-model prior helps most at low SNR}
($+41\%$ at $-5$~dB when removed) where the observation is least trustworthy; (iii) a
...
\midrule
$-5$~dB & $0.0484$ & $0.0684$ & $0.0477$ & $\mathbf{0.0441}$ & $0.1882$ \\
$0$~dB  & $0.0207$ & $0.0251$ & $0.0205$ & $\mathbf{0.0177}$ & $0.0799$ \\
$5$~dB  & $0.0093$ & $0.0104$ & $0.0093$ & $\mathbf{0.0079}$ & $0.0365$ \\
$10$~dB & $0.0040$ & $0.0042$ & $0.0040$ & $\mathbf{0.0034}$ & $0.0159$ \\
$20$~dB & $\mathbf{0.0009}$ & $0.0010$ & $0.0010$ & $\mathbf{0.0009}$ & $0.0033$ \\
$30$~dB & $\mathbf{0.0005}$ & $0.0005$ & $0.0005$ & $0.0005$ & $0.0017$ \\
\bottomrule
\end{tabular}
\caption{In-distribution full-grid estimation NMSE with one module removed (lower better), $8\times4$
MIMO. Removing beamspace is catastrophic ($+248$--$298\%$); the prior helps most at low SNR
($+41\%$ at $-5$~dB when removed); a persistence prior is competitive for the \emph{estimation} gate
but not for prediction (Table~\ref{tab:pred}).}
\label{tab:ablation}
\end{table}
...
does not dominate a well-tuned per-snapshot CNN; its estimation value is concentrated in the
sparse-pilot/moderate-SNR corner, while its clear, unconditional win is \emph{prediction}. (2) The
sparse-pilot advantage is regime-specific (the router falls back to the CNN at dense pilots and SNR
extremes), and we observed in earlier single-ended experiments that it narrows as the spatial
