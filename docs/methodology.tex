...
\subsection{Sparse-pilot estimation: when the prior becomes load-bearing}\label{sec:sparse}
The ablation above shows the world-model prior is only a modest help when the estimator observes the
\emph{full} noisy grid --- the fusion gate can lean on the observation and largely ignore the prior.
The realistic regime is different: practical OFDM estimates the channel from \emph{sparse comb
pilots} and must fill the unobserved subcarriers. We model this with a $1$-in-$s$ comb mask: the
observation is zeroed on non-pilot subcarriers, and fusion runs in the antenna--subcarrier domain
with a \emph{mask-aware} gate $g_{\text{eff}}=g\odot\text{mask}$, so unobserved subcarriers are forced
onto the world-model prior (the dense channel decoded from the beamspace prediction). Encoder, SSM,
and predictor stay in beamspace.

Here the prior is decisively load-bearing, and its value \emph{grows as pilots thin}
(Table~\ref{tab:sparse}): removing it (\emph{no prior}, same mask-aware head) costs, averaged over
SNR, $+29\%$ at $50\%$ pilots, $+76\%$ at $25\%$, and $+148\%$ at $12.5\%$ --- at $12.5\%$ pilots the
full model is $\sim$$2.5\times$ better than without the prior, and the OOD lift ($+69\%$ at $25\%$)
matches the in-distribution one. This resolves the ablation puzzle: the gate ignored the prior under
dense pilots because it \emph{could}; deny it a full observation and the temporal prior is what fills
the gap. Against baselines the picture is honest and mixed: the model beats classical linear pilot
interpolation at every density and SNR, but a masked linear-MMSE interpolator \emph{handed the true
frequency covariance and noise power} is strong and wins at most SNRs --- the world model overtakes it
only in the hardest corner (very sparse $\times$ low SNR, e.g.\ $12.5\%$ pilots at $-5$ and $0$~dB,
marked $^\ast$), where the fixed-covariance MMSE degrades.

\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
& \multicolumn{4}{c}{avg.\ over SNR $[-5,30]$~dB} \\
pilots (stride) & interp & masked-MMSE & \textbf{full} & no prior \\
\midrule
$50\%$ ($s{=}2$)   & $0.51$ & $0.15$ & $0.09$ & $0.13$ \quad($+29\%$) \\
$25\%$ ($s{=}4$)   & $0.48$ & $0.09$ & $0.11$ & $0.19$ \quad($+76\%$) \\
$12.5\%$ ($s{=}8$) & $0.51$ & $0.13$ & $0.14^\ast$ & $0.35$ \quad($+148\%$) \\
$25\%$ OOD         & $0.48$ & $0.10$ & $0.12$ & $0.20$ \quad($+69\%$) \\
\bottomrule
\end{tabular}
\caption{Sparse-pilot estimation NMSE (SNR-averaged, lower better). The world-model prior's lift over
an identical no-prior head \emph{grows} as pilots thin ($+29\!\to\!+76\!\to\!+148\%$). The model beats
linear interpolation everywhere; oracle-covariance masked-MMSE wins at most SNRs, and the model
overtakes it only in the very-sparse/low-SNR corner ($^\ast$).}
\label{tab:sparse}
\end{table}
